...
const generateToken = (user) => {
  return jwt.sign({ id: user.id, email: user.email, role: user.role }, process.env.SECRET_KEY, {
    expiresIn: "24h",
  });
};
class UserController {
  async registration(req, res, next) {
    ...
    if (condidate) {
      return next(new ErrorHandler(404, `Пользователь с таким ${email} уже существует`));
    }
